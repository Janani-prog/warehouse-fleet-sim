...
CAUSAL_EVAL_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "results" / "causal_eval"
...
@app.get("/api/runs/{run_id}/agent_decisions")
def get_agent_decisions(run_id: str) -> list[dict]:
    """The M8 closed loop's per-triggered-tick decision log (M10's Agent
    Action Log view) - empty list for a pre-M8 run that has no
    agent_decisions.parquet, same graceful-empty pattern as every other
    per-run table here."""
    return _read_parquet_records(_run_dir(run_id) / "agent_decisions.parquet")


@app.get("/api/causal_eval")
def get_causal_eval() -> dict:
    """Not run-scoped, like /api/forecaster/thresholds - there's one current
    M9 causal-eval report (data/results/causal_eval/), regenerated by
    `python -m eval.run_causal_eval`, not one per simulator run."""
    report_path = CAUSAL_EVAL_DIR / "causal_eval_report.json"
    if not report_path.exists():
        raise HTTPException(status_code=404, detail="causal evaluation has not been run yet")
